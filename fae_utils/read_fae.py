...
if __name__ == '__main__':
    ...
    # Let's point to relocation entries
    iterator = crt0_size + FAEConstants.BINARY_SIZE_BYTESIZE
    ...
    if relocation_entries_size > 0:
        ...
        iterator += relocation_entries_size
    else:
        print('- Relocation entries : none.')

    # Text alignment
    align_text = 0
    padding_value_as_int = int.from_bytes(FAEConstants.TEXT_ALIGNMENT_PADDING_VALUE)
    while ( fae_memoryview.obj[iterator] == padding_value_as_int ):
        iterator += 1
        align_text += 1

    print_bytesize('- Text alignment', align_text)

    print_bytesize('- Offset to partition', iterator)
    # 2 : Entrypoint
