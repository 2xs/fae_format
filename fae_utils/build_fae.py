...
CLI_OPTION_CRT0_PATH = "--crt0-path"
CLI_OPTION_ALIGN_TEXT = "--align-text"

def usage():
    """Print how to to use the script and exit"""
    print(f'usage: {sys.argv[0]} [{CLI_OPTION_CRT0_PATH} crt0_path] '
          f'[{CLI_OPTION_ALIGN_TEXT} integer_value] ELFFilename')
    print('')
    print(f'{sys.argv[0]} will build :')
    print( '    - a fae file from ELFFilename,')
    print(f'    - a {FAEConstants.GDBINIT_FILENAME} file, to be used with gdb after editing.')
    ...
    print(f'    Default is {FAEConstants.CRT0_DEFAULT_PATH}')
    print('')
    print(f'{CLI_OPTION_ALIGN_TEXT} positive_or_equal_to_zero_integer_value')
    print( '    This option allows to pad the embedded elf text section to a power of two in order to respect sections and variables alignment')
    print('    Default is 0')
    sys.exit(1)
...
def export_crt0_to_bytearray(path_to_crt0, to_bytearray):
    ...
    if (path_to_crt0 is None):
        path_to_crt0 = FAEConstants.CRT0_DEFAULT_PATH.strip()

        if path_to_crt0.endswith('/') == False:
            path_to_crt0 += '/'
...
def export_relocation_table(elf_file, relocation_table_name, to_bytearray):
    ...
    for i, entry in enumerate(sh.iter_relocations()):
        if get_r_type(entry['r_info']) != r_types['R_ARM_ABS32']:
            die(f'export_relocation_table : {relocation_table_name} : entry {i}: unsupported relocation type ({get_r_type(entry["r_info"])})')
...
def compute_header_size(crt0_bytearray, relocation_bytearray):
    return len(crt0_bytearray) + FAEConstants.BINARY_SIZE_BYTESIZE + len(relocation_bytearray)

def compute_text_alignment(crt0_bytearray, relocation_bytearray, align_text):
    header_size = compute_header_size(crt0_bytearray, relocation_bytearray)
    if (align_text > 0):
        align_text = round(header_size, align_text) - header_size
    return align_text
...
def concatenate_and_pad_bytearray(
    ...
    raw_binary_size = header_size + align_text + len(partition_bytearray)
    ...
    # Text alignment
    print(f'Align text : {align_text} bytes')
    for i in range(align_text):
        to_bytearray += FAEConstants.TEXT_ALIGNMENT_PADDING_VALUE
    print(f'Offset to partition : {len(to_bytearray)} bytes')
    # Partition
...
if __name__ == '__main__':
    ...
    if argc == 2:
        ...
        elif (sys.argv[1] == CLI_OPTION_ALIGN_TEXT):
            align_text = int(sys.argv[2].strip())
        else:
            usage()
        ...
        if (sys.argv[1] == CLI_OPTION_CRT0_PATH) and (sys.argv[3] == CLI_OPTION_ALIGN_TEXT):
            crt0_path  = sys.argv[2].strip()
            align_text = int(sys.argv[4].strip())
        elif (sys.argv[1] == CLI_OPTION_ALIGN_TEXT) and (sys.argv[3] == CLI_OPTION_CRT0_PATH):
            align_text = int(sys.argv[2].strip())
            crt0_path  = sys.argv[4].strip()
        else:
            usage()
    ...
    output_filename = elf_filename_parts[0] + FAEConstants.FAE_SUFFIX
    ...
    with open(elf_filename, 'rb') as f:
        ...
        export_relocation_tables(                              \
            ...
            relocation_bytearray)
        align_text = compute_text_alignment(crt0_bytearray, relocation_bytearray, align_text)
        metadata_size = compute_header_size(crt0_bytearray, relocation_bytearray) + align_text
